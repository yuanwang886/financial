...
public class LoginService {
	 ...
	 */
	@Transactional
	public User registerByAdmin(String realname, String phone, String password, String invitePhone)
			throws Exception {
		if (StringUtils.isEmpty(phone) || StringUtils.isEmpty(password) || StringUtils.isEmpty(invitePhone)) {
			throw new Exception("请输入手机号,密码以及邀请人(A117)");
		}

		// 首先校验手机合法性
		if (phone.length() != 11) {
			throw new Exception("手机号码不合法(A116)");
		}

		if (invitePhone.length() != 11) {
			throw new Exception("推荐人手机号码不合法(A115)");
		}

		if (StringUtils.equals(phone, invitePhone)) {
			throw new Exception("注册账号不能与推荐人手机号码一致(A114)");
		}
